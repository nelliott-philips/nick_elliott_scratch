...
root = "C:\\autodump\\pv018\\" # save path to dump the data. Should be updated and use '\\' for file seperators and surrounded by ""

subDirChroma = 'ACE\\CF\\PV018\\PV018_COnlyMode_1Frame_Noise_PreAccum.bin' # subdirectory in the VDiag root that paths to the memory loaded test patterns for chromaflo, loads in A frame
subDirBmode = 'ACE\\CF\\PV018\\PV018_BOnlyMode_1Frame_Noise_PreAccum.bin' # subdirectory in the VDiag root that paths to the memory loaded test patterns for bmode, loads in B frame

if not os.path.exists(root):
   os.makedirs(root)

# load in the PV.018 bootmode and ensure some test patterns are working before running this.
while not GuiTest.toolTabRdn('bm3'):
	time.sleep(3)
...
# Enable env data saves, record only A frame for the CF.
GuiTest.runTabCbx('env',True)
GuiTest.bufferSet('A')

# cf full, by def. all CF settings are def.
# 37.5 - Like 38 (MM) but RD not turned on.
